"use strict";
/*
 * A JavaScript implementation of the Secure Hash Algorithm, SHA-1, as defined in FIPS 180-1 Version 2.2 Copyright Paul
 * Johnston 2000 - 2009. Other contributors: Greg Holt, Andrew Kepert, Ydnar, Lostinet Distributed under the BSD License
...
/**
 * Exposes the SHA1 library. The <code>hex</code>, <code>b64</code>, <code>any</code>, <code>hmacHex</code>,<code>hmacB64</code>,
...
exports.sha1 = {
	...
	hex : hex_sha1,
	/**
	 * Hashes an object and returns the base 64 value
	 *
	 * @function
	 */
	b64 : b64_sha1,
	 ...
	 * @function
	 */
	hmacHex : hex_hmac_sha1,

	/**
	 * Performs an HMAC and returns the base 64 value
	 *
	 * @function
	 */
	hmacB64 : b64_hmac_sha1,
	/**
	 * Performs an HMAC
	 *
	 * @function
	...
	hash : function( plaintext ) {
		if ( sys.isEmpty( plaintext ) ) { throw new Error( " hash plaintext required" ); }

		return hex_sha1( plaintext );
	}
};
...
function rstr2any( input, encoding ) {
	 ...
	 * divisor. Once computed, the quotient forms the dividend for the next step. We stop when the dividend is zero. All
	 * remainders are stored for later use.
	 */
...
function binb_sha1( x, len ) {
	...
	for ( var i = 0; i < x.length; i += 16 ) {
		...
		for ( var j = 0; j < 80; j++ ) {
			...
			var t = safe_add( safe_add( bit_rol( a, 5 ), sha1_ft( j, b, c, d ) ), safe_add( safe_add( e, w[ j ] ),
					sha1_kt( j ) ) );
			e = d;
			d = c;
			c = bit_rol( b, 30 );
